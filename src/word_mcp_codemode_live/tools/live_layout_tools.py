    ...
    try:
        ...
        with undo_record(app, "MCP: Add Header/Footer"):
            ...
            if header_text is not None:
                ...
                hdr.Range.ParagraphFormat.Alignment = align_map.get(header_alignment.lower(), 1)
                added.append("header")

            if footer_text is not None:
                ftr = section.Footers(1)  # Primary footer
                ftr.Range.Text = footer_text
                ftr.Range.ParagraphFormat.Alignment = align_map.get(footer_alignment.lower(), 1)
                added.append("footer")
...
async def word_live_add_watermark(
    ...
    Args:
        filename: Document name or path (None = active document).
        text: Watermark text (e.g. "TASLAK", "DRAFT", "GÄ°ZLÄ°").
