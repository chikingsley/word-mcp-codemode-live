...
from word_mcp_codemode_live.core.styles import ensure_heading_style
from word_mcp_codemode_live.utils.document_utils import (
...
async def add_table(
    filename: str, rows: int, cols: int, data: list[list[str]] | None = None
) -> str:
...
async def add_picture(filename: str, image_path: str, width: float | None = None) -> str:
    ...
    try:
        ...
        return (
            f"Document processing error: {error_type} - {error_msg or 'No error details available'}"
        )
...
async def replace_paragraph_block_below_header_tool(
    filename: str, header_text: str, new_paragraphs: list, detect_block_end_fn=None
) -> str:
    """Reemplaza el bloque de pÃ¡rrafos debajo de un encabezado, evitando modificar TOC."""
    async with get_file_lock(filename):
        return replace_paragraph_block_below_header(
            filename, header_text, new_paragraphs, detect_block_end_fn
        )


async def replace_block_between_manual_anchors_tool(
    filename: str,
    start_anchor_text: str,
    new_paragraphs: list,
    end_anchor_text: str | None = None,
    match_fn=None,
    new_paragraph_style: str | None = None,
) -> str:
