...
from word_mcp_codemode_live.core.styles import create_style
from word_mcp_codemode_live.core.tables import (
...
from word_mcp_codemode_live.core.tables import set_table_width as set_table_width_func
from word_mcp_codemode_live.utils.document_utils import get_effective_text
...
async def format_text(
    ...
    Args:
        ...
        color: Text color â€” named ("red", "blue") or hex ("FF0000").
    ...
    try:
        ...
        return (
            "Invalid parameter: paragraph_index, start_pos, end_pos, and font_size must be integers"
        )
...
async def apply_table_alternating_rows(
    filename: str, table_index: int, color1: str = "FFFFFF", color2: str = "F2F2F2"
) -> str:
...
async def highlight_table_header(
    filename: str, table_index: int, header_color: str = "4472C4", text_color: str = "FFFFFF"
) -> str:
...
async def merge_table_cells(
    filename: str, table_index: int, start_row: int, start_col: int, end_row: int, end_col: int
) -> str:
...
async def merge_table_cells_horizontal(
    filename: str, table_index: int, row_index: int, start_col: int, end_col: int
) -> str:
...
async def merge_table_cells_vertical(
    filename: str, table_index: int, col_index: int, start_row: int, end_row: int
) -> str:
...
async def set_table_cell_alignment(
    filename: str,
    table_index: int,
    row_index: int,
    col_index: int,
    ...
    try:
        async with get_file_lock(filename):
            ...
            success = set_cell_alignment_by_position(
                table, row_index, col_index, horizontal, vertical
            )
...
async def set_table_alignment_all(
    filename: str, table_index: int, horizontal: str = "left", vertical: str = "top"
) -> str:
...
async def set_table_column_width(
    filename: str, table_index: int, col_index: int, width: float, width_type: str = "points"
) -> str:
    ...
    try:
        ...
        return (
            "Invalid parameter: table_index and col_index must be integers, width must be a number"
        )
...
async def set_table_column_widths(
    filename: str, table_index: int, widths: list, width_type: str = "points"
) -> str:
...
async def set_table_width(
    filename: str, table_index: int, width: float, width_type: str = "points"
) -> str:
    ...
    try:
        async with get_file_lock(filename):
            ...
            if success:
                doc.save(filename)
                return (
                    f"Table width set successfully for table {table_index} to {width} {width_type}."
                )
            else:
...
async def format_table_cell_text(
    filename: str,
    table_index: int,
    row_index: int,
    col_index: int,
    ...
    try:
        async with get_file_lock(filename):
            ...
            success = format_cell_text_by_position(
                table,
                row_index,
                col_index,
                text_content,
            ...
            if success:
                ...
                if text_content is not None:
                    format_desc.append(
                        f"content='{text_content[:30]}{'...' if len(text_content) > 30 else ''}'"
                    )
                if bold is not None:
...
async def set_table_cell_padding(
    filename: str,
    table_index: int,
    row_index: int,
    col_index: int,
    ...
    try:
        async with get_file_lock(filename):
            ...
            # Apply cell padding
            success = set_cell_padding_by_position(
                table, row_index, col_index, top, bottom, left, right, word_unit
            )
