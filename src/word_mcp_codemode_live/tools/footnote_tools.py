...
from word_mcp_codemode_live.core.footnotes import (
    add_footnote_robust,
    ...
    try:
        async with get_file_lock(filename):
            ...
            try:
                ...
                footnote.add_footnote(footnote_text)  # ty: ignore[unresolved-attribute]
                ...
                last_run.text = "Â¹"  # Unicode superscript 1
                ...
                if not found_footnote_section:
                    doc.add_paragraph("\n").add_run()
                    heading = doc.add_paragraph("Footnotes:")
                    heading.runs[0].bold = True

                # Add footnote text
                footnote_para = doc.add_paragraph("Â¹ " + footnote_text)
...
async def add_endnote_to_document(filename: str, paragraph_index: int, endnote_text: str) -> str:
    ...
    try:
        async with get_file_lock(filename):
            ...
            last_run = paragraph.add_run()
            last_run.text = "â€ "  # Unicode dagger symbol common for endnotes
            last_run.font.superscript = True
            ...
            endnote_para = doc.add_paragraph("â€  " + endnote_text)
        ...
        async with get_file_lock(filename):
            ...
            for para_idx, para in enumerate(doc.paragraphs):
                for run_idx, run in enumerate(para.runs):
                    ...
                    if run.font.superscript and (
                        run.text.isdigit() or run.text in "Â¹Â²Â³â´âµâ¶â·â¸â¹"
                    ):
                        footnote_references.append(
                            {"paragraph_index": para_idx, "run_index": run_idx, "text": run.text}
                        )
            ...
            for i, ref in enumerate(footnote_references):
                ...
                if i < len(footnote_text):
                    endnote_para.text = f"â€ {i + 1} {footnote_text[i]}"
                else:
                    endnote_para.text = f"â€ {i + 1} Converted from footnote {ref['text']}"

                # Change the footnote reference to an endnote reference
                try:
                    paragraph = doc.paragraphs[ref["paragraph_index"]]
                    paragraph.runs[ref["run_index"]].text = f"â€ {i + 1}"
                except IndexError:
...
async def add_footnote_after_text(
    filename: str, search_text: str, footnote_text: str, output_filename: str | None = None
) -> str:
...
async def add_footnote_before_text(
    filename: str, search_text: str, footnote_text: str, output_filename: str | None = None
) -> str:
...
async def add_footnote_enhanced(
    filename: str, paragraph_index: int, footnote_text: str, output_filename: str | None = None
) -> str:
...
async def delete_footnote_from_document(
    filename: str,
...
async def validate_footnotes_tool(filename: str) -> dict[str, Any]:
    ...
    if not os.path.exists(filename):
        return {"valid": False, "message": f"Document {filename} does not exist", "report": {}}

    # Call validation
    is_valid, message, report = validate_document_footnotes(filename)

    return {"valid": is_valid, "message": message, "report": report}
...
async def add_footnote_to_document_robust(
    ...
    result = await add_footnote_robust_tool(
        filename=filename, paragraph_index=paragraph_index, footnote_text=footnote_text
    )
...
async def add_footnote_after_text_robust(
    ...
    result = await add_footnote_robust_tool(
        filename=working_file, search_text=search_text, footnote_text=footnote_text
    )
    return result["message"]
...
async def delete_footnote_from_document_robust(
    ...
    result = await delete_footnote_robust_tool(
        filename=working_file, footnote_id=footnote_id, search_text=search_text
    )
    return result["message"]
