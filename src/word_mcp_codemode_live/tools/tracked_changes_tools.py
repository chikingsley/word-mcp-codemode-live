...
from word_mcp_codemode_live.core.tracked_changes import (
...
async def track_replace(
    ...
    This changes TEXT CONTENT only â€” it does not change formatting (font, highlight, style).
...
async def track_insert(
    ...
    This changes TEXT CONTENT only â€” it does not change formatting.
...
async def accept_tracked_changes(
    ...
    try:
        ...
        return json.dumps(
            {"success": False, "error": f"Failed to accept tracked changes: {str(e)}"}
        )
...
async def reject_tracked_changes(
    ...
    try:
        ...
        return json.dumps(
            {"success": False, "error": f"Failed to reject tracked changes: {str(e)}"}
        )
