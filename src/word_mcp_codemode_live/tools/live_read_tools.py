...
async def word_live_take_snapshot(filename: str | None = None) -> str:
...
async def word_live_get_diff(filename: str | None = None) -> str:
...
async def word_live_snapshot_status(filename: str | None = None) -> str:
    ...
    try:
        ...
        for i in range(start_paragraph, end_paragraph + 1):
            ...
            info = {
                ...
                "line_spacing_rule": SPACING_RULE_NAMES.get(
                    fmt.LineSpacingRule, str(fmt.LineSpacingRule)
                ),
                "left_indent_pt": fmt.LeftIndent,
                "right_indent_pt": fmt.RightIndent,
                "first_line_indent_pt": fmt.FirstLineIndent,
                "page_break_before": bool(fmt.PageBreakBefore),
                "keep_with_next": bool(fmt.KeepWithNext),
                "keep_together": bool(fmt.KeepTogether),
            }
            try:
                info["horizontal_position_pt"] = rng.Information(5)
            except Exception:
                pass
...
async def word_live_find_text(
    ...
    try:
        ...
        while len(matches) < max_results:
            ...
            # Move past current match â€” guard against transient COM failures
            try:
...
async def word_live_resolve_comment(
    ...
    try:
        ...
        try:
            ...
            return json.dumps({"error": "Comment resolve/unresolve requires Word 2016 or later."})
        except Exception as e:
            if "not available" in str(e).lower():
                return json.dumps(
                    {
                        "error": "Comment.Done is not available â€” Word's Modern Comments "
...
async def word_live_get_undo_history(
    ...
    try:
        ...
        try:
            ...
            # Must specify Type=6 (msoControlSplitDropdown) â€” without it,
            ...
            # Undocumented API â€” may not be available in all Word versions
            return json.dumps(
                {
                    "success": True,
                    "document": doc.Name,
...
async def word_live_diagnose_layout(
    ...
    try:
        ...
        for i in range(1, total_paras + 1):
            ...
            is_heading = style_name.lower().startswith("heading") or style_name.startswith(
                "BaÅŸlÄ±k"
            )
        ...
        heading_count = sum(
            ...
            if k.lower().startswith("heading") or k.startswith("BaÅŸlÄ±k")
        )
...
async def word_live_list_open() -> str:
    ...
    try:
        ...
        # ActiveDocument access can throw on broken/proxy state â€” degrade gracefully.
        try:
        ...
        for i in range(1, count + 1):
            ...
            # Defensive per-property access â€” one broken doc must not
