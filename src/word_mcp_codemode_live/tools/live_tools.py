...
async def word_live_insert_text(
    ...
    Args:
        ...
        text: Text to insert (no length limit â€” auto-chunked if needed).
    ...
    """
    if _MAC_AVAILABLE:
        from word_mcp_codemode_live.core.word_mac import mac_insert_text

        return mac_insert_text(
            filename=filename,
            text=text,
            position=position,
            bookmark=bookmark,
    ...
    try:
        ...
        # Reject control bytes (notably \x07 cell separator) â€” inserting
...
async def word_live_format_text(
    ...
    Args:
        ...
        paragraph_alignment: Paragraph alignment â€” "left" (0), "center" (1), "right" (2), "justify" (3).
        ...
        preserve_direct_formatting: When True and style_name is set, saves font/size/bold/italic/
            ...
            a paragraph's style (e.g., Heading 5 â†’ Normal) without losing its visual formatting.
    ...
    """
    if _MAC_AVAILABLE:
        from word_mcp_codemode_live.core.word_mac import mac_format_text
    ...
    try:
        ...
        with undo_record(app, "MCP: Format Text"):
            ...
            try:
                ...
                saved_formats: list[dict[str, Any]] = []
                if preserve_direct_formatting and style_name is not None:
                    for para in rng.Paragraphs:
                        ...
                        saved_formats.append(
                            {
                                "para": para,
                                "font_name": str(pr.Font.Name)
                                if pr.Font.Name and pr.Font.Name != 9999999
                                else None,
                                "font_size": pr.Font.Size
                                if pr.Font.Size and pr.Font.Size != 9999999
                                else None,
                                "bold": pr.Font.Bold if pr.Font.Bold != 9999999 else None,
                                "italic": pr.Font.Italic if pr.Font.Italic != 9999999 else None,
                                "strikethrough": pr.Font.StrikeThrough
                                if pr.Font.StrikeThrough != 9999999
                                else None,
                ...
                if paragraph_alignment is not None:
                    ...
                    if al is None:
                        return json.dumps(
                            {
                                "error": f"Invalid alignment: {paragraph_alignment}. Use: left, center, right, justify"
                            }
                        )
                    for para in rng.Paragraphs:
...
async def word_live_apply_list(
    ...
    Args:
        ...
        number_format: (multilevel only) Dict mapping level (int) to format string.
            Example: {1: "4.%1.", 2: "(%2)", 3: "(%3)"} â†’ "4.1.", "(a)", "(i)"
        ...
        start_at: (multilevel only) Dict mapping level (int) to starting number.
            Example: {1: 5} â†’ numbering starts at 5.
            If not provided, starts at 1.
        level_map: (multilevel only) Dict for per-paragraph list level. Two formats:
            1) {para_index (int): level (int)} â€” numeric paragraph indices.
            2) {heading_text (str): level (int)} â€” match by text, robust to index shifts.
    ...
    """
    if _MAC_AVAILABLE:
        from word_mcp_codemode_live.core.word_mac import mac_apply_list
        ...
        return mac_apply_list(
            ...
            track_changes=track_changes,
            font_color=font_color,
            outline_numbered=outline_numbered,
        )
            ...
            try:
                ...
                if remove:
                    ...
                    for lvl_num, fmt_str in nf.items():
                        ...
                        lv.TextPosition = 28
                        lv.TabPosition = 28
                        # Do NOT set LinkedStyle â€” avoids Heading style side effects
...
async def word_live_setup_heading_numbering(
    ...
    Custom formats supported â€” e.g., h1_number_format="MADDE %1 â€“ " produces "MADDE 1 â€“ ".
    ...
    - MADDE: "MADDE 6 â€“ ", "MADDE 10 - " (regex: ^MADDE\s+\d+\s*[â€“-]\s*)
    ...
    Args:
        ...
        h1_number_format: Custom Level 1 format (default "%1."). Use %1 for the number.
            Example: "MADDE %1 â€“ " produces "MADDE 1 â€“ ", "MADDE 2 â€“ ", etc.
        ...
        alignment: Paragraph alignment â€” "left", "center", "right", "justify".
    ...
    if _MAC_AVAILABLE:
        from word_mcp_codemode_live.core.word_mac import mac_setup_heading_numbering
        ...
        with undo_record(app, "MCP: Setup Heading Numbering"):
            ...
            if has_style_params:
                ...
                for style_id, size, sp_before, sp_after in [
                    ...
                    # H2 does NOT â€” sub-clauses are often full paragraphs;
            ...
            if len(h1_fmt) > 5:
                # Long format (e.g., "MADDE %1 â€“ ") â€” text follows number directly
                lv1.TextPosition = 0
                lv1.TabPosition = 0
            else:
            ...
            def _apply_direct_formatting(rng, sid):
                ...
                customizations onto the range itself â€” this defeats any
            ...
            for para_idx, style_id in all_heading_paras:
                if para_idx < 1 or para_idx > doc.Paragraphs.Count:
                    restyle_failures.append({"index": para_idx, "error": "out of range"})
                    continue
                ...
                applied_via = None
                if not inflated:
                    # Normal paragraph â€” direct style assignment works.
                    try:
                    ...
                    restyle_failures.append(
                        {
                            ...
                            "error": (
                                "style assignment did not stick â€” paragraph "
            ...
            if strip_manual_numbers:
                strip_patterns = [
                    r"^MADDE\s+\d+\s*[â€“\-]\s*",  # "MADDE 6 â€“ ", "MADDE 10 - "
                    r"^\d+(\.\d+)*\.?\s+",  # "1. ", "6.2. ", "10.3 "
                ]
                    ...
                    for pattern in strip_patterns:
                        ...
                        if m:
                            ...
                            if range_len <= len(text) + 5:
                                # Normal â€” para.Range.Start is reliable
                                rng = doc.Range(
                                    para.Range.Start,
                                    para.Range.Start + prefix_len,
                                )
                            else:
                                # Inflated â€” find text to get real position
                                found = _find_para_text(doc, text)
                                if not found:
                                    break
                                rng = doc.Range(
...
async def word_live_replace_text(
    ...
    """
    if _MAC_AVAILABLE:
        from word_mcp_codemode_live.core.word_mac import mac_replace_text

        return mac_replace_text(
            filename=filename,
            find_text=find_text,
        ...
        return json.dumps(
            {
                ...
                "Use replace_all=False â€” each unique text only needs one replacement."
            }
        )
    ...
    try:
        ...
        with undo_record(app, "MCP: Replace Text"):
            ...
            if track_changes:
                ...
                # untracked replace_all â€” disable temporarily to prevent
            ...
            try:
                ...
                while True:
                    ...
                    if not found:
                        break
                    # Guard: zero-length match â†’ skip forward 1 char to avoid infinite loop
                    ...
                    if count >= MAX_REPLACEMENTS:
                        break
                    rng.Collapse(0)  # wdCollapseEnd â€” move past replacement
            finally:
                doc.TrackRevisions = prev_tracking
                if track_changes:
                    app.UserName = prev_author
...
async def word_live_insert_paragraphs(
    ...
    try:
        ...
        if target_paragraph_index is not None:
            com_index = target_paragraph_index + 1  # 0-based API â†’ 1-based COM
...
async def word_live_add_table(
    ...
    """
    if _MAC_AVAILABLE:
        from word_mcp_codemode_live.core.word_mac import mac_add_table
    ...
    try:
        ...
        if position == "start":
            ...
            # existing table's range â€” Word would silently merge the
...
async def word_live_format_table(
    ...
    try:
        ...
        if idx < 1 or idx > doc.Tables.Count:
            return json.dumps(
                {"error": f"Table index {table_index} out of range (1-{doc.Tables.Count})"}
            )
        ...
        with undo_record(app, "MCP: Format Table"):
            ...
            if border_style is not None:
                ...
                if style_val is None:
                    return json.dumps(
                        {
                            "error": f"Unknown border_style: {border_style}. Use: {list(BORDER_STYLES.keys())}"
                        }
                    )
            ...
            if autofit is not None:
                AUTOFIT = {
                    ...
                    "content": 1,
                    "fixed": 0,
                }  # wdAutoFitWindow=2, wdAutoFitContent=1, wdAutoFitFixed=0
            ...
            if left_indent_points is not None:
                tbl.Rows.LeftIndent = float(left_indent_points)
                actions.append(f"left_indent_points={left_indent_points}")

            # --- Table font ---
            if font_name is not None:
                tbl.Range.Font.Name = font_name
                actions.append(f"font_name={font_name}")
            if font_size is not None:
            ...
            if cell_shading is not None:
                for entry in cell_shading:
                    ...
                    def shade_cell(row_i, col_i, *, _bgr=bgr):
                        tbl.Cell(row_i, col_i).Shading.BackgroundPatternColor = _bgr
...
async def word_live_delete_text(
    ...
    """
    if _MAC_AVAILABLE:
        from word_mcp_codemode_live.core.word_mac import mac_delete_text
...
async def word_live_modify_table(
    ...
    Args:
        ...
        accept_revisions: For set_cell/set_row/set_range â€” accept tracked changes before writing
            (prevents layered text from old revisions persisting underneath new content).
        track_changes: Track modifications as revisions.
        scrub_orphans: For delete_table â€” scan the deletion site for orphan
    ...
    """
    if _MAC_AVAILABLE:
        from word_mcp_codemode_live.core.word_mac import mac_modify_table

        return mac_modify_table(
            filename=filename,
    ...
    try:
        ...
        # get_info is read-only â€” no undo record needed
        if op == "get_info":
        ...
        with undo_record(app, "MCP: Modify Table"):
            ...
            try:
                if op == "set_cell":
                    ...
                    result = table_com.set_cell(
                        table, row, col, text, accept_revisions=accept_revisions
                    )

                elif op == "set_row":
                    if row is None or not cells:
                        return json.dumps(
                            {"error": "set_row requires row and cells (list of values)"}
                        )
                    ...
                    if not all(v is not None for v in [start_row, start_col, end_row, end_col]):
                        return json.dumps(
                            {"error": "merge_cells requires start_row, start_col, end_row, end_col"}
                        )
...
async def word_live_undo(
    ...
    """
    if _MAC_AVAILABLE:
        from word_mcp_codemode_live.core.word_mac import mac_undo
...
async def word_live_open(filename: str) -> str:
    ...
    try:
        ...
        from word_mcp_codemode_live.core.word_com import remember_word_app
...
async def word_live_go_to_page(
    ...
    """[Windows only] Navigate an open Word document to a page.

    Args:
        page: 1-based page number to display.
        filename: Document name or path (None = active document).
        scroll_lines: Additional lines to scroll after navigation; positive scrolls down.

    Returns:
        JSON with the displayed page and document name.
    """
    if sys.platform != "win32":
        return json.dumps({"error": "Live navigation is only available on Windows"})
    if page < 1:
    ...
    try:
        ...
        document.Activate()
        target = document.GoTo(What=1, Which=1, Count=page)
        target.Select()
        document.ActiveWindow.ScrollIntoView(target, True)
        if scroll_lines > 0:
            document.ActiveWindow.SmallScroll(Down=scroll_lines)
        elif scroll_lines < 0:
            document.ActiveWindow.SmallScroll(Up=abs(scroll_lines))
        return json.dumps(
            {
                "success": True,
                "document": document.Name,
...
async def word_live_save(
    ...
    """
    if _MAC_AVAILABLE:
        from word_mcp_codemode_live.core.word_mac import mac_save
...
async def word_live_toggle_track_changes(
    ...
    """
    if _MAC_AVAILABLE:
        from word_mcp_codemode_live.core.word_mac import mac_toggle_track_changes
    ...
    try:
        ...
        if wrapping is not None:
            ...
            if wrapping.lower() != "inline" and wrap_val is None:
                return json.dumps(
                    {"error": f"Unknown wrapping: {wrapping}. Use: {list(WRAP_STYLES.keys())}"}
                )
            ...
            if wrap_val is not None:
                ...
                if alignment is not None:
                    ...
                    if al in ALIGN_MAP:
                        ...
                        float_shape.RelativeHorizontalPosition = (
                            0  # wdRelativeHorizontalPositionMargin
                        )
                        float_shape.RelativeVerticalPosition = (
                            2  # wdRelativeVerticalPositionParagraph
                        )
                        page_setup = doc.PageSetup
                        text_width = (
                            page_setup.PageWidth - page_setup.LeftMargin - page_setup.RightMargin
                        )
                        if al == "left":
                            float_shape.Left = 0
                        elif al == "right":
...
async def word_live_insert_cross_reference(
    ...
    if _MAC_AVAILABLE:
        return json.dumps(
            {"error": "word_live_insert_cross_reference is not yet implemented on macOS"}
        )
...
async def word_live_list_cross_reference_items(
    ...
    if _MAC_AVAILABLE:
        return json.dumps(
            {"error": "word_live_list_cross_reference_items is not yet implemented on macOS"}
        )
    ...
    try:
        ...
        if ref_type_lower == "heading":
            ...
            seq_label = {"figure": "Figure", "table": "Table", "equation": "Equation"}[
                ref_type_lower
            ]
            idx = 1
            for i in range(1, doc.Paragraphs.Count + 1):
...
async def word_live_insert_equation(
    ...
    UNICODE_MATH = {
        ...
        r"\Im": "\u2124",
        r"\aleph": "\u2135",
        # Matrix (Word UnicodeMath uses â–  for matrix)
        r"\matrix": "\u25a0",
        r"\pmatrix": "\u25a0",
