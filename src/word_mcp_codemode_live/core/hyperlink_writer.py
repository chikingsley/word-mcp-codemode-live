...
def _load_zip_part(zip_bytes: bytes, part_name: str) -> bytes | None:
...
def add_hyperlink_to_doc(
    ...
    if paragraph_index is not None:
        if paragraph_index < 0 or paragraph_index >= len(paragraphs):
            return {
                "success": False,
                "error": f"Paragraph index {paragraph_index} out of range (0-{len(paragraphs) - 1})",
            }
    ...
    hyperlink_elem.set(f"{{{R_NS}}}id", rid)
