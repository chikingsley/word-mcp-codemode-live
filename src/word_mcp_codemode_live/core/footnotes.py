...
def _ensure_footnote_styles(styles_root):
    ...
    if not ref_style:
        ...
        style = etree.Element(
            f"{{{W_NS}}}style",
            attrib={f"{{{W_NS}}}type": "character", f"{{{W_NS}}}styleId": "FootnoteReference"},
        )
    ...
    if not text_style:
        ...
        style = etree.Element(
            f"{{{W_NS}}}style",
            attrib={f"{{{W_NS}}}type": "paragraph", f"{{{W_NS}}}styleId": "FootnoteText"},
        )
        name = etree.SubElement(style, f"{{{W_NS}}}name")
    ...
    try:
        ...
        with zipfile.ZipFile(filename, "r") as zin:
            ...
            if "word/styles.xml" in zin.namelist():
                ...
                doc_parts["styles"] = (
                    b'<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"/>'
                )
        ...
        if search_text:
            ...
            if paragraph_index is None:
                return False, "paragraph_index is required when search_text is omitted", None
        ...
        new_footnote = etree.Element(
            f"{{{W_NS}}}footnote", attrib={f"{{{W_NS}}}id": str(footnote_id)}
        )
        ...
        with zipfile.ZipFile(temp_file, "w", zipfile.ZIP_DEFLATED) as zout:
            ...
            zout.writestr(
                "word/footnotes.xml",
                etree.tostring(
                    footnotes_root, encoding="UTF-8", xml_declaration=True, standalone="yes"
                ),
            )
            zout.writestr(
                "word/styles.xml",
...
def delete_footnote_robust(
    ...
    try:
        ...
        for fn_ref in doc_root.xpath(
            f'//w:footnoteReference[@w:id="{footnote_id}"]', namespaces=nsmap
        ):
...
def validate_document_footnotes(filename: str) -> tuple[bool, str, dict[str, Any]]:
    ...
    try:
        with zipfile.ZipFile(filename, "r") as zf:
            ...
            if "word/_rels/document.xml.rels" in zf.namelist():
                ...
                fn_rels = rels_root.xpath(
                    "//r:Relationship[contains(@Type, 'footnotes')]", namespaces=rel_nsmap
                )

                if report["total_content"] > 0 and len(fn_rels) == 0:
                    report["coherence_issues"].append("Missing footnotes relationship")
                elif len(fn_rels) > 1:
                    report["coherence_issues"].append(
                        f"Multiple footnote relationships: {len(fn_rels)}"
                    )
            ...
            if "[Content_Types].xml" in zf.namelist():
                ...
                fn_overrides = ct_root.xpath(
                    "//ct:Override[@PartName='/word/footnotes.xml']", namespaces=ct_nsmap
                )
                ...
                if report["total_content"] > 0 and len(fn_overrides) == 0:
                    ...
                    report["coherence_issues"].append(
                        f"Multiple footnote content types: {len(fn_overrides)}"
                    )
            ...
            if "word/styles.xml" in zf.namelist():
                ...
                ref_style = styles_root.xpath(
                    '//w:style[@w:styleId="FootnoteReference"]', namespaces=nsmap
                )
                text_style = styles_root.xpath(
                    '//w:style[@w:styleId="FootnoteText"]', namespaces=nsmap
                )
        ...
        # Determine if valid
        is_valid = (
            len(report["id_conflicts"]) == 0
            and len(report["orphaned_content"]) == 0
            and len(report["missing_references"]) == 0
            and len(report["invalid_locations"]) == 0
            and len(report["coherence_issues"]) == 0
        )
