...
def _get_run_rpr(run: etree._Element) -> etree._Element | None:
...
def _make_run(text: str, rpr: etree._Element | None = None, is_del: bool = False) -> etree._Element:
...
def _find_text_in_paragraph(p: etree._Element, search_text: str) -> list[tuple] | None:
...
def reject_tracked_changes_in_doc(
    ...
    # Reject deletions: unwrap <w:del>, convert delTextâ†’t (restore original text)
    for del_elem in list(root.iter(W("del"))):
        if not _should_process(del_elem):
            continue
        parent = del_elem.getparent()
