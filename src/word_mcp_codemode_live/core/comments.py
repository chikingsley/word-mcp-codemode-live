...
def extract_all_comments(doc: DocumentType) -> list[dict[str, Any]]:
    ...
    try:
        ...
        comments_part = None
        for _rel_id, rel in document_part.rels.items():
        ...
        for row in table.rows:
            for cell in row.cells:
                for para_idx, paragraph in enumerate(cell.paragraphs):
                    para_comments = find_paragraph_comments(
                        paragraph, para_idx, comment_id, in_table=True
                    )
                    comments.extend(para_comments)
                    comment_id += len(para_comments)
...
def extract_comment_data(comment_element, index: int) -> dict[str, Any] | None:
    """
    ...
    try:
        ...
        comment_id = comment_element.get(
            "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}id", str(index)
        )
        author = comment_element.get(
            "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}author", "Unknown"
        )
        initials = comment_element.get(
            "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}initials", ""
        )
        date_str = comment_element.get(
...
def find_paragraph_comments(
    paragraph: Paragraph, para_index: int, start_id: int, in_table: bool = False
) -> list[dict[str, Any]]:
    """
    ...
    try:
        ...
        if "commentRangeStart" in xml_text or "commentReference" in xml_text:
            ...
            comment_info = {
                ...
                "in_table": in_table,
                "reference_text": paragraph.text[:50] + "..."
...
def get_comments_for_paragraph(
    comments: list[dict[str, Any]], paragraph_index: int
) -> list[dict[str, Any]]:
    """
