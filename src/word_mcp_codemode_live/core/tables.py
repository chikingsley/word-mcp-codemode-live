        ...
        if border_style:
            ...
            for row in table.rows:
                for cell in row.cells:
                    set_cell_border(
                        cell, top=True, bottom=True, left=True, right=True, val=val, color="000000"
                    )
...
def set_cell_shading_by_position(table, row_index, col_index, fill_color, pattern="clear"):
    ...
    """
    try:
        if 0 <= row_index < len(table.rows) and 0 <= col_index < len(table.rows[row_index].cells):
...
def merge_cells(table, start_row, start_col, end_row, end_col):
    ...
    try:
        ...
        if (
            ...
            or end_row < 0
            or end_col < 0
            or start_row >= len(table.rows)
            or end_row >= len(table.rows)
        ...
        for row_idx in range(start_row, end_row + 1):
            if start_col >= len(table.rows[row_idx].cells) or end_col >= len(
...
def format_cell_text_by_position(
    table,
    row_index,
    col_index,
    text_content=None,
    ...
    try:
        if 0 <= row_index < len(table.rows) and 0 <= col_index < len(table.rows[row_index].cells):
            cell = table.rows[row_index].cells[col_index]
            return format_cell_text(
                cell, text_content, bold, italic, underline, color, font_size, font_name
            )
        else:
            return False
    except Exception as e:
...
def set_cell_padding(cell, top=None, bottom=None, left=None, right=None, unit="dxa"):
    ...
    try:
        ...
        if any(p is not None for p in [top, bottom, left, right]):
            ...
            margin_sides = {"w:top": top, "w:bottom": bottom, "w:left": left, "w:right": right}
...
def set_cell_padding_by_position(
    table, row_index, col_index, top=None, bottom=None, left=None, right=None, unit="dxa"
):
    """
