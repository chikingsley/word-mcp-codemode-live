...
def find_text(
    ...
    try:
        ...
        for i, para in enumerate(doc.paragraphs):
            # Prepare text for comparison â€” use get_effective_text for tracked-change support
                            ...
                            results["occurrences"].append(
                                {
                                    "paragraph_index": i,
                                    "position": word_idx,
                                    "context": effective[:100]
                                    + ("..." if len(effective) > 100 else ""),
                                }
                            )
        ...
        for table_idx, table in enumerate(doc.tables):
            for row_idx, row in enumerate(table.rows):
                for col_idx, cell in enumerate(row.cells):
                    for _para_idx, para in enumerate(cell.paragraphs):
...
def get_highlighted_text(doc_path: str, color: str | None = None) -> dict[str, Any]:
    """
        ...
        for t_idx, table in enumerate(doc.tables):
            for r_idx, row in enumerate(table.rows):
                for c_idx, cell in enumerate(row.cells):
                    _check_paragraphs(cell.paragraphs, f"Table {t_idx}, Row {r_idx}, Col {c_idx}")
