...
def get_document_properties(doc_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "word_count": sum(
                len(get_effective_text(paragraph).split()) for paragraph in doc.paragraphs
            ),
    ...
    try:
        ...
        for i, para in enumerate(doc.paragraphs):
            structure["paragraphs"].append(
                {
                    "index": i,
                    "text": get_effective_text(para)[:100]
                    + ("..." if len(get_effective_text(para)) > 100 else ""),
                ...
                for run in runs:
                    ...
                    if rpr is not None:
                        if (
                            ...
                            or rpr.find(qn("w:caps")) is not None
                            or rpr.find(qn("w:sz")) is not None
                        ):
                            end_idx = i
                            break
